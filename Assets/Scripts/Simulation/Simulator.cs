    ...
    public static class Simulator
	{
		...
		static void ProcessBuiltinChip(SimChip chip)
		{
			switch (chip.ChipType)
			{
				...
				case ChipType.DisplayRGB8BitColor:
				{
					const uint addressSpace = 256;
					uint addressPin = chip.InputPins[0].State.GetShortValues();
					bool writePin = chip.InputPins[5].State.SmallHigh();
					...
					if (isRisingEdge)
					{
						...
						{
							uint data = (chip.InputPins[1].State.GetShortValues() | (chip.InputPins[2].State.GetShortValues() << 8) | (chip.InputPins[3].State.GetShortValues() << 16));
							chip.InternalState[addressPin + addressSpace] = data;
						}
					...
					// Output current pixel colour
					uint colData = chip.InternalState[addressPin];
					chip.OutputPins[0].State.SetShort((ushort)(colData & 0b11111111));//red
					chip.OutputPins[1].State.SetShort((ushort)((colData >> 8) & 0b11111111));//green
					chip.OutputPins[2].State.SetShort((ushort)((colData >> 16) & 0b11111111));//blue
				...
				case ChipType.DisplayRGBTouch8BitColor:
				{
					const uint addressSpace = 256;
					uint addressPin = chip.InputPins[0].State.GetShortValues();
					bool writePin = chip.InputPins[5].State.SmallHigh();
