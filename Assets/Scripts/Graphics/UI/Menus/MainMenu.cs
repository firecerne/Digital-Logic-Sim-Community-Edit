...
namespace DLS.Graphics
{
    public static class MainMenu
    {
        ...
        static void DrawLoadProjectScreen()
        {
            ...
            {
                bool buttonEnabled = activePopup == PopupKind.None && (compatibleProject || i == backButtonIndex || (i == deleteButtonIndex && projectSelected) || i == importButtonIndex);
            ...
            else if (buttonIndex == importButtonIndex)
            {
#if UNITY_EDITOR
                string zipPath = UnityEditor.EditorUtility.OpenFilePanel("Select DLS Project Zip File", "", "zip");
                if (!string.IsNullOrEmpty(zipPath))
                {
                    string extractPath = SavePaths.ProjectsPath;
                    ...
                    try
                    {
                        ...
                        UI.GetScrollbarState(ID_ProjectsScrollView).scrollY = 0;
                    }
            ...
            else if (buttonIndex == exportButtonIndex) activePopup = PopupKind.ExportProject;
        }
        ...
        static void OnNamePopupConfirmed(PopupKind kind, string name)
        {
            ...
            else if (kind is PopupKind.ExportProject)
            {
                // Not implemented
            }
        }
        ...
        static void DrawExportProjectPopup()
        {
            ...
            {
                Draw.ID panelID = UI.ReservePanel();
                UI.DrawText($"Export project '{SelectedProjectName}' as zip file?", theme.FontRegular, theme.FontSizeRegular, UI.Centre, Anchor.Centre, Color.white);
                ...
                {
                    activePopup = PopupKind.None;
                    ExportProject(SelectedProjectName);
                }
            }
        }
        ...
        {
#if UNITY_EDITOR
            string defaultFileName = $"{projectName}.zip";
            string savePath = UnityEditor.EditorUtility.SaveFilePanel("Export DLS Project", "", defaultFileName, "zip");
            if (!string.IsNullOrEmpty(savePath))
            {
                ...
                if (System.IO.Directory.Exists(projectPath))
                {
                    try
                    {
                        ...
                        Debug.Log($"Successfully exported project '{projectName}' to '{savePath}'");
                    }
                    catch (System.Exception e)
                    {
                        Debug.LogError($"Error exporting project: {e.Message}");
                    }
                }
                else
                {
...
#else
            Debug.LogWarning("Project export is only available in the Unity Editor");
#endif
        }
